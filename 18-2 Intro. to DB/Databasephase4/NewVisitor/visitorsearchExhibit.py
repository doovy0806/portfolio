...
class Ui_searchExhibit(object):
    ...
    def loadExhibit(self):
        ...
        query = ("select Name, Size, numAnimals, Water_Feature  from zoo.exhibit join"
                 " (select count(*) as numAnimals, animal.Exhibit from zoo.animal group by animal.Exhibit) as a "
    ...
    def exhibitSearch(self):
        ...
        if water == "No":
            query = ("select Name, Size, numAnimals, Water_Feature  from zoo.exhibit join (select count(*) as numAnimals, animal.Exhibit from zoo.animal group by animal.Exhibit) as a on exhibit.name = a.exhibit where name like '%{}%' and size>={} and size<={} and numAnimals>={} and numAnimals<={} and water_feature = {};").format(exhibitName, sizeMin, sizeMax, numMin, numMax, 'False')
        elif water == "Yes":
            query = ("select Name, Size, numAnimals, Water_Feature  from zoo.exhibit join (select count(*) as numAnimals, animal.Exhibit from zoo.animal group by animal.Exhibit) as a on exhibit.name = a.exhibit where name like '%{}%' and size>={} and size<={} and numAnimals>={} and numAnimals<={} and water_feature = {};").format(exhibitName, sizeMin, sizeMax, numMin, numMax, 'True')
        else:
            query = ("select Name, Size, numAnimals, Water_Feature  from zoo.exhibit join"+
                     " (select count(*) as numAnimals, animal.Exhibit from zoo.animal group by animal.Exhibit) as a "+
