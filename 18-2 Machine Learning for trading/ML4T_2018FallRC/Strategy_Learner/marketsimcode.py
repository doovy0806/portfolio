...
def compute_portvals(orders_df, symbol='JPM',  start_val=1000000, commission=9.95, impact=0.005 ):
    ...
    stockvals = get_data([symbol], pd.date_range(start_date, end_date))
    stockvals = stockvals[[symbol]]  # remove SPY
    spxvals = get_data([symbol], pd.date_range(start_date, end_date))
    spxvals = stockvals[[symbol]]  # remove SPY
    porvals = get_data([symbol], pd.date_range(start_date, end_date))
    porvals = stockvals[[symbol]]  # remove SPY

    rv = stockvals.copy()
    rv.ix[:] = 0

    cash_amount = start_val
    stock_index = {symbol: 1}
    symbols = [symbol]
    ...
    for day in rv.index:
        order = orders_df.ix[day,symbol]

        if order > 0 and portfolio[1]<=0:

            price = stockvals.ix[day]*(1+ impact)
            portfolio[1] += order
            portfolio[0] -= order*price + commission
        elif order<0 and portfolio[1] >= 0:
