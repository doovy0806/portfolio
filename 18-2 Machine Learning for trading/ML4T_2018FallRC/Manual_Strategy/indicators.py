...
def bollingerbands(pricedf):
    ...
    jpm_rm = pd.rolling_mean(normalizedprice['JPM'], window=10)
    jpm_rstd = pd.rolling_std(normalizedprice['JPM'], window=10)
    jpm_bbup = jpm_rm+(2*jpm_rstd)
    jpm_bbdown = jpm_rm - (2*jpm_rstd)
    bb_jpm = (normalizedprice['JPM'] - jpm_rm) / (2*jpm_rstd)
    return bb_jpm
    # plt.figure(1)
...
def smadiff(pricedf):
    normalizedprice = pricedf / pricedf.ix[0]
    jpm_rm = pd.rolling_mean(normalizedprice['JPM'], window=10)
    jpm_rm20 = pd.rolling_mean(normalizedprice['JPM'], window=20)
    jpmsma = normalizedprice['JPM']-jpm_rm
